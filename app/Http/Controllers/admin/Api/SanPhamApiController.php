...
class SanPhamApiController extends Controller
{
    ...
    public function searchByBarcode(Request $request): JsonResponse
    {
        ...
        $sp = SanPham::with([
            'bienThe' => fn($q) => $q->where('loai_bien_the', 'don_vi'),
            'bienThe.donVi',
        ])
        ...
        if (!$sp) {
            return response()->json(['success' => false, 'message' => 'Khong tim thay san pham']);
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'bien_the_don_vi' => $sp->bienThe->map(fn($bt) => [
                    'id' => $bt->id,
                    'ten_don_vi' => $bt->donVi->ten_don_vi ?? '',
                    'so_luong_quy_doi' => $bt->donVi->so_luong_san_pham_trong_don_vi ?? 1,
                    'gia_ban' => $bt->gia_ban,
                    'ma_vach' => $bt->ma_vach,
