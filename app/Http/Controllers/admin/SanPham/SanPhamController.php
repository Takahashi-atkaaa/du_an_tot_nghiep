...
class SanPhamController extends Controller
{
    ...
    public function edit($id): View
    {
        ...
        return view('admin_xem_truoc.san-pham.sua', [
            ...
            'productData' => $product->toEditVueData(),
        ]);
    }
    ...
    protected function syncUnits(int $variantId, array $units, ?string $uploadedImage): void
    {
        $incomingUnitIds = [];
        // Lay anh hien tai cua variant neu khong co anh moi upload
        $variantImage = $uploadedImage ?? BienTheSanPham::find($variantId)?->hinh_anh;

        foreach ($units as $unit) {
            $unitId = $unit['id'] ?? null;

            if ($unitId && DonViQuyDoi::where('id', $unitId)->where('variant_id', $variantId)->exists()) {
                // Cap nhat
                DonViQuyDoi::find($unitId)->update([
                    'ten_don_vi' => $unit['ten_don_vi'],
                    'ty_le_quy_doi' => (int)($unit['ty_le_quy_doi'] ?? 1),
                    'ma_vach' => $unit['ma_vach'] ?? null,
                    'gia_von_quy_doi' => $unit['gia_von_quy_doi'] ?? 0,
