            ...
            <div class="card-body">
                <form id="form-sua-phieu-xuat-full">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="loai_xuat" class="form-select" id="edit-px-loai" required>
                                <option value="tra_hang_nha_cung_cap" {{ $phieuXuat->loai_xuat === 'tra_hang_nha_cung_cap' ? 'selected' : '' }}>Trả hàng NCC</option>
                                <option value="tieu_huy" {{ $phieuXuat->loai_xuat === 'tieu_huy' ? 'selected' : '' }}>Tiêu hủy</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        @endif
                                        <br><small class="text-muted">Mã: {{ $ct->variant?->ma_vach ?? '--' }}</small>
                                    </td>
                    ...
                    <div class="alert alert-info py-2 small mt-3">
                        ...
                        <strong>Lưu ý:</strong> Chỉ có thể sửa thông tin cơ bản (loại xuất, NCC, lý do). Chi tiết sản phẩm đã xuất không thể thay đổi sau khi tạo phiếu.
                    </div>
                </form>
            </div>
            ...
            <div class="card-body small">
                <table class="table table-sm mb-0">
                    ...
                    <tr>
                        ...
                        <td class="fw-bold text-danger">{{ number_format($phieuXuat->chiTietPhieu->sum('so_luong')) }}</td>
                    </tr>
                </table>
            </div>
        </div>
...
$(function () {
    ...
    $.get('/admin/api/lo-hang/nha-cung-cap', function (res) {
        ...
        const opts = res.map(n => `<option value="${n.id}" ${n.id == nccId ? 'selected' : ''}>${n.ten_nha_cung_cap}</option>`).join('');
    ...
    $('#btn-luu-sua-phieu-xuat-full').click(function () {
        ...
        $.ajax({
            ...
            error: function (x) {
                hienThongBaoXuatEdit('danger', x.responseJSON?.message || 'Đã xảy ra lỗi.');
            }
        });
    });
});
