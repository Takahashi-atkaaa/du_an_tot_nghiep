            ...
            @if(isset($items) && $items->count())
                @foreach($items as $promo)
                    @php
                        ...
                        $loaiKey = strtolower((string) $promo->loai_giam_gia);
                        $loaiLabel = $loaiMap[$loaiKey] ?? ucfirst(str_replace(['_', '-'], ' ', $promo->loai_giam_gia));

                        if (in_array($loaiKey, ['percent', 'phan_tram', 'percentage'])) {
                            $badge = 'Giảm ' . rtrim(rtrim(number_format($promo->gia_tri_giam, 2, ',', '.'), '0'), ',') . '%';
                        } elseif (in_array($loaiKey, ['amount', 'fixed', 'tien_mat', 'so_tien'])) {
                        ...
                        <div class="card table-admin h-100">
                            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                                @if(in_array($loaiKey, ['percent', 'phan_tram', 'percentage']))
                                    <span class="badge bg-primary">
                                        <i class="fas fa-percent me-1"></i>{{ $badge }}
                                    </span>
                                @elseif(in_array($loaiKey, ['amount', 'fixed', 'tien_mat', 'so_tien']))
                                    <span class="badge bg-success">
                                        <i class="fas fa-coins me-1"></i>{{ $badge }}
                                    </span>
                                @elseif($loaiKey === 'bogo')
                                    <span class="badge bg-info text-dark">
                                        <i class="fas fa-gifts me-1"></i>{{ $loaiLabel }}
                                    </span>
                                @elseif(in_array($loaiKey, ['gift', 'qua_tang']))
                                    <span class="badge bg-warning text-dark">
                                        <i class="fas fa-gift me-1"></i>{{ $loaiLabel }}
                                    </span>
                                @else
                                    <span class="badge bg-secondary">
                                        <i class="fas fa-tag me-1"></i>{{ $loaiLabel }}
                                    </span>
                                @endif
                            ...
                            <div class="card-body">
                                ...
                                <div class="d-flex align-items-center text-muted small mb-2">
                                    ...
                                    <span>
                                        {{ $promo->ngay_bat_dau ? $promo->ngay_bat_dau->format('d/m/Y') : '-' }}
                                        -
                                        {{ $promo->ngay_ket_thuc ? $promo->ngay_ket_thuc->format('d/m/Y') : '-' }}
                                    </span>
                                </div>
                                ...
                                <div class="d-flex align-items-center text-muted small">
                                    ...
                                    <span>
                                        {{ $promo->so_luong_sp_toi_thieu ? $promo->so_luong_sp_toi_thieu . ' sp tối thiểu' : 'Áp dụng cho tất cả sản phẩm' }}
                                    </span>
                                </div>
                            </div>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" name="so_luong_sp_toi_thieu" class="form-control" placeholder="0">
                        </div>
