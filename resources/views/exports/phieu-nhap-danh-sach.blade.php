@php
    ...
    $title = $hasRange
        ? "BÁO CÁO NHẬP HÀNG từ {$tuLabel} đến {$denLabel}"
        : 'BÁO CÁO NHẬP HÀNG (toàn bộ thời gian)';
@endphp
...
<p style="margin: 0 0 12px 0; text-align: center; font-size: 11px; color: #666;">
    @if(!empty($filters['loai_nhap']))
        Loại: {{ $filters['loai_nhap'] === 'mua_hang' ? 'Nhập mua hàng' : 'Trả lại từ khách' }}
        |
    @endif
    ...
    <tbody>
        ...
        <tr>
            <td colspan="10" style="text-align: center; border: 1px solid black;">
                @if($hasRange)
                    Không có phiếu nhập nào trong khoảng từ {{ $tuLabel }} đến {{ $denLabel }}.
                @else
                    Không có dữ liệu
                @endif
            </td>
